...
import net.phoenix492.testmod.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    private static final Map<Holder<ArmorMaterial>, List<MobEffectInstance>> MATERIAL_TO_EFFECT_MAP =
        (new ImmutableMap.Builder<Holder<ArmorMaterial>, List<MobEffectInstance>>())
            .put(ModArmorMaterials.BISMUTH_ARMOR_MATERIAL, List.of(
                ...
                new MobEffectInstance(MobEffects.GLOWING, 200, 1, false, true))
            )
            .build();
    ...
    private void evaluateArmorEffects(Player player) {
        MATERIAL_TO_EFFECT_MAP.forEach((mapArmorMaterial, mapEffect) -> {
            if (playerHasCorrectArmorOn(mapArmorMaterial, player)) {
    ...
    private boolean playerHasCorrectArmorOn(Holder<ArmorMaterial> mapArmorMaterial, Player player) {
